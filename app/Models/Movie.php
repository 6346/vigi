...
class Motorcycle extends Model
{
    ...
    public $with = ['genres', 'languages', 'countries', 'model', 'images'];
    ...
    public static function customCreate(Request $request): self
    {
        return DB::transaction(function () use ($request) {
            ...
            $Motorcycle = self::create($inputs);
            $Motorcycle->syncAll($request);

            //Upload and insert multiple images 
            if ($images = $request->file('images')) {
                $images = $Motorcycle->uploadImages($images);
                $Motorcycle->insertImages($images);
            }

            //Upload cover image 
            if ($image = $request->file('image')) {
                $images = $Motorcycle->uploadImages([$image]);
            }
    ...
    public function customUpdate(Request $request): self
    {
        DB::transaction(function () use ($request) {
            ...
            //Detach old images
            MotorcycleImage::where('Motorcycle_id', $this->id)->whereNotIn('name', $oldImages)->forceDelete();
    ...
    public function insertImages($images): self
    {
        collect($images)->each(function (string $item, int $key) {
            MotorcycleImage::updateOrCreate([
                'name' => $item,
    ...
    public function syncAll(Request $request): self
    {
        ...
        $this->model()->sync($request->get('model'));
